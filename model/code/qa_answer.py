...
tf.app.flags.DEFINE_float("matching_units", 30, "Number of matching units.")
tf.app.flags.DEFINE_float("learning_rate", 0.01, "Learning rate.")
tf.app.flags.DEFINE_float("dropout", 0.15, "Fraction of units randomly dropped on non-recurrent connections.")
tf.app.flags.DEFINE_integer("batch_size", 10, "Batch size to use during training.")
tf.app.flags.DEFINE_integer("epochs", 10, "Number of epochs to train.")
tf.app.flags.DEFINE_integer("state_size", 70, "Size of each model layer.")
tf.app.flags.DEFINE_integer("embedding_size", 100, "Size of the pretrained vocabulary.")
tf.app.flags.DEFINE_integer("question_output_size", 30, "The output size of your question.")
tf.app.flags.DEFINE_integer("paragraph_output_size", 400, "The output size of your paragraph.")
...
def initialize_model(session, model, train_dir):
    ...
    if ckpt and (tf.gfile.Exists(ckpt.model_checkpoint_path) or tf.gfile.Exists(v2_path)):
        ...
        saver.restore(session, train_dir+"/savings.ckpt")
    else:
...
def generate_answers(sess, model, dataset, rev_vocab):
    ...
    for i in range(len(dataset[0])):
        paragraph = dataset[0][i]
        question = dataset[1][i]
        question_uuid = dataset[2][i]
        q_id = map(int, question.split(" "))
        p_id = map(int, paragraph.split(" "))
        q_batch, m_q_batch = pad_sequences(q_id, FLAGS.question_output_size)
        p_batch, m_p_batch = pad_sequences(p_id, FLAGS.paragraph_output_size)
        q_batch = np.reshape(q_batch, (1, FLAGS.question_output_size))
        m_q_batch = np.reshape(m_q_batch, (1, FLAGS.question_output_size))
        p_batch = np.reshape(p_batch, (1, FLAGS.paragraph_output_size))
        m_p_batch = np.reshape(m_p_batch, (1, FLAGS.paragraph_output_size))
        a_s, a_e = model.answer(sess, q_batch, p_batch, m_q_batch, m_p_batch)
        index_list  = p_id[a_s[0]:a_e[0] + 1]
        str_list = []
        for i in range(len(index_list)):
            str_list.append(rev_vocab[index_list[i]])
        ans = " ".join(str_list)
        answers[question_uuid] = ans
